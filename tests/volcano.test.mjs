...
test('Charizard guards use only species authored for the volcanic region',()=>{
  ...
  const wildSpawns=sim.map.spawns.filter(spawn=>spawn.zoneId==='vulcao');
  assert.equal(guards.length,6);
  assert.ok(wildSpawns.length>0);
  assert.ok(guards.every(enemy=>volcanicSpecies.has(enemy.id)));
  assert.ok(wildSpawns.every(spawn=>volcanicSpecies.has(spawn.species)));

...
test('exact Gen 3 and EBDX resources replace local effects without empty sheets',async()=>{
 const rows=Object.entries(EXTERNAL_MOVE_VFX);assert.equal(rows.length,233);assert.equal(rows.filter(([,row])=>row.source==='gen3').length,54);assert.equal(rows.filter(([,row])=>row.source==='ebdx').length,179);assert.equal(rows.filter(([,row])=>row.audio).length,230);assert.equal(EXTERNAL_MOVE_VFX.earthquake.source,'ebdx');
 for(const [id,row] of rows){const path=fileURLToPath(new URL(`../public/assets/sprites/vfx/moves/${id}.png`,import.meta.url)),{data,info}=await sharp(path).ensureAlpha().raw().toBuffer({resolveWithObject:true});assert.equal(info.width,512,id);assert.equal(info.height,64,id);let visible=0;for(let index=3;index<data.length;index+=4)if(data[index]>8)visible++;assert.ok(visible>info.width*info.height*.002,id);if(row.audio)assert.ok(existsSync(new URL(`../public/assets/audio/moves/external/${row.audio}`,import.meta.url)),row.audio);const visual=moveAnimationVisual(ABILITIES[id]);assert.equal(visual.travel,`moves/${id}`);assert.equal(visual.external,row.source);}
});

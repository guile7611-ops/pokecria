...
export function encounterStats(species,map,position,uid,{boss=false,horde=false,respawn=0}={}){
 ...
 const pathRange=levelRangeAt(map,position);const distance=Math.hypot(position.x-240,position.y-560),band=1+Math.floor(distance/700);
 ...
 const low=Math.max(pathRange?.[0]??region.level[0],pathRange?1:band,Number(position.minLevel)||1,!pathRange&&region.id==='vulcao'&&evolved?40:1);
 const high=pathRange?Math.max(low,pathRange[1]):Math.max(low+2,region.level[1],region.id==='vulcao'?(evolved?80:70):low+Math.max(4,Math.round(low*.32)));

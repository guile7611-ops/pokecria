...
function renderMoves(){
 ...
 section.innerHTML=`<div class="info-section-heading"><span class="info-section-title">GOLPES APRENDIDOS</span><span class="info-section-rule" aria-hidden="true"></span></div><small class="move-help">Selecione um atalho para equipar o golpe. O dano final considera poder, defesa, tipagem e acertos.</small><div class="move-list">${(p.knownMoves||[]).map(id=>{const a=ABILITIES[id],equipped=p.slots.indexOf(id),style={area:'Área',zone:'Área persistente',channel:'Canalização',beam:'Raio carregado',whip:'Chicote',wave:'Onda',rolling:'Controle',buff:'Buff',debuff:'Efeito',teleport:'Movimento',heal:'Cura',direct:'Contato',projectile:'Projétil'}[a.behavior]||'Golpe';return `<article class="move-card ${equipped>=0?'equipped':''}"><span class="move-art" style="background-image:url('/assets/sprites/vfx/${a.vfx}.png')" aria-hidden="true"></span><div class="move-copy"><div class="move-heading"><b>${a.name}</b>${equipped>=0?`<em>EQUIPADO · ${KEYS[equipped]}</em>`:''}</div><span>${a.type||'Normal'} <i>◆</i> ${style} <i>◆</i> ${a.cooldown}s</span><strong class="move-power">${baseDamageLabel(a)}</strong><small>${a.description}</small></div><div class="move-slots">${KEYS.map((key,index)=>`<button data-move="${id}" data-move-slot="${index}" aria-label="Equipar ${a.name} em ${key}" ${equipped===index||(index===2&&p.level<10)||(index===3&&p.level<25)?'disabled':''}>${key}</button>`).join('')}</div></article>`}).join('')}</div>`;
 $('info-body').append(section);
 section.querySelectorAll('[data-move]').forEach(button=>button.onclick=()=>{if(sim.equipMove(button.dataset.move,Number(button.dataset.moveSlot))){const scroll=$('info-body').scrollTop;renderInfo();addTypeInfo();renderMoves();renderGuild();$('info-body').scrollTop=scroll;saveWorld(localStorage,sim);note(`${ABILITIES[button.dataset.move].name} equipado em ${KEYS[button.dataset.moveSlot]}.`);}});
 const catalog=document.createElement('details');catalog.className='move-catalog';catalog.innerHTML=`<summary>CATÁLOGO DE GOLPES · ${MOVE_CATALOG.length}</summary><p>Golpes desta espécie aprendidos por nível aparecem acima; outros aguardam MT, MH ou novos sistemas.</p><div class="move-catalog-grid">${MOVE_CATALOG.map(move=>`<article><span class="move-preview" style="background-image:url('/assets/sprites/vfx/${ABILITIES[move.id].vfx}.png')"></span><div><b>${move.name}</b><small>${move.type} · ${move.category} · ${move.source}</small><strong class="move-power">${baseDamageLabel(move)}</strong><small>${move.description}</small></div></article>`).join('')}</div>`;$('info-body').append(catalog);
}

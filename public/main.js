...
const online = new OnlineClient({
  ...
  'wild-hit':state=>{if(Number.isFinite(state.hp))sim.applySharedWildState({...state,respawnAt:state.hp<=0?Date.now()+(state.respawn||13)*1000:0});},
  'wild-kill':state=>sim.applySharedWildState({...state,hp:0,respawnAt:Date.now()+(state.respawn||13)*1000}),
...
function renderMoves(){
 const p=sim.player,section=document.createElement('section'),known=(p.knownMoves||[]).filter(id=>ABILITIES[id]);section.className='move-manager';
 const moveCard=id=>{const a=ABILITIES[id],equipped=p.slots.indexOf(id),style={area:'Área',zone:'Área persistente',channel:'Canalização',beam:'Raio carregado',whip:'Chicote',wave:'Onda',rolling:'Controle',buff:'Buff',debuff:'Efeito',teleport:'Movimento',heal:'Cura',direct:'Contato',projectile:'Projétil'}[a.behavior]||'Golpe',search=[id,a.name,a.type||'Normal',style].join(' ').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();return `<article class="move-card ${equipped>=0?'equipped':''}" data-move-card data-move-search="${safeText(search)}"><span class="move-art" style="background-image:url('/assets/sprites/vfx/${a.vfx}.png')" aria-hidden="true"></span><div class="move-copy"><div class="move-heading"><b>${a.name}</b>${equipped>=0?`<em>EQUIPADO · ${KEYS[equipped]}</em>`:''}</div><span>${a.type||'Normal'} <i>◆</i> ${style} <i>◆</i> ${a.cooldown}s</span><strong class="move-power">${baseDamageLabel(a)}</strong><small>${a.description}</small></div><div class="move-slots">${KEYS.map((key,index)=>`<button data-move="${id}" data-move-slot="${index}" aria-label="Equipar ${a.name} em ${key}" ${equipped===index||(index===2&&p.level<10)||(index===3&&p.level<25)?'disabled':''}>${key}</button>`).join('')}</div></article>`};
 section.innerHTML=`<div class="info-section-heading"><span class="info-section-title">GOLPES APRENDIDOS · ${known.length}</span><span class="info-section-rule" aria-hidden="true"></span></div><small class="move-help">Selecione um atalho para equipar o golpe. O dano final considera poder, defesa, tipagem e acertos.</small>${known.length>12?`<label class="move-search"><span>BUSCAR GOLPE</span><input type="search" placeholder="Nome, tipo ou comportamento" autocomplete="off"><b>${known.length}</b></label>`:''}<div class="move-list">${known.map(moveCard).join('')}</div><p class="move-empty" hidden>Nenhum golpe corresponde à busca.</p>`;
 $('info-body').append(section);
 section.querySelectorAll('[data-move]').forEach(button=>button.onclick=()=>{if(sim.equipMove(button.dataset.move,Number(button.dataset.moveSlot))){const scroll=$('info-body').scrollTop,query=section.querySelector('.move-search input')?.value||'';renderInfo();addTypeInfo();renderMoves();renderGuild();const nextSearch=$('info-body').querySelector('.move-search input');if(nextSearch&&query){nextSearch.value=query;nextSearch.dispatchEvent(new Event('input'));}$('info-body').scrollTop=scroll;saveWorld(localStorage,sim);note(`${ABILITIES[button.dataset.move].name} equipado em ${KEYS[button.dataset.moveSlot]}.`);}});
 const search=section.querySelector('.move-search input');if(search)search.oninput=()=>{const query=search.value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().toLowerCase();let visible=0;section.querySelectorAll('[data-move-card]').forEach(card=>{card.hidden=!card.dataset.moveSearch.includes(query);if(!card.hidden)visible++;});section.querySelector('.move-search b').textContent=visible;section.querySelector('.move-empty').hidden=visible>0;};
 const knownSet=new Set(known),unlearned=MOVE_CATALOG.filter(move=>!knownSet.has(move.id));if(unlearned.length){const catalog=document.createElement('details');catalog.className='move-catalog';catalog.innerHTML=`<summary>CATÁLOGO DE GOLPES · ${unlearned.length}</summary><p>Golpes desta espécie aprendidos por nível aparecem acima; outros aguardam MT, MH ou novos sistemas.</p><div class="move-catalog-grid">${unlearned.map(move=>`<article><span class="move-preview" style="background-image:url('/assets/sprites/vfx/${ABILITIES[move.id].vfx}.png')"></span><div><b>${move.name}</b><small>${move.type} · ${move.category} · ${move.source}</small><strong class="move-power">${baseDamageLabel(move)}</strong><small>${move.description}</small></div></article>`).join('')}</div>`;$('info-body').append(catalog);}
}
...
function toggleInfo(){if($('info').open){$('info').close();return;}held=false;arrows.clear();sim.player.path=[];sim.player.target=null;renderInfo();addTypeInfo();renderMoves();renderGuild();$('info').showModal();}
...
function frame(now) {
  ...
  if (started) {
    ...
    for (const e of sim.events.splice(0)) {
      ...
      if(online.token&&e.type==='damage'&&Number.isInteger(e.uid)){const target=sim.enemies.find(v=>v.uid===e.uid);online.request('wild-hit',{uid:e.uid,damage:e.amount,isBoss:!!target?.isBoss,hp:target?.hp,maxHp:target?.maxHp,stateVersion:target?.stateVersion,respawn:target?.respawn}).catch(()=>{});}
      if(online.token&&e.type==='kill'&&Number.isInteger(e.uid)){const target=sim.enemies.find(v=>v.uid===e.uid);online.request('wild-kill',{uid:e.uid,xp:e.xp,hp:0,maxHp:target?.maxHp,stateVersion:target?.stateVersion,respawn:target?.respawn}).catch(()=>{});}

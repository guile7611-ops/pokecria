...
export const MOVE_LOADOUT_VERSION = 2;
...
export class Simulation {
  constructor(starter = 'bulbasaur', options = {}) {
    ...
    this.spawnConfig={...DEFAULT_SPAWN_CONFIG,...options.spawnConfig,rarityWeights:{...DEFAULT_SPAWN_CONFIG.rarityWeights,...options.spawnConfig?.rarityWeights}};
    this.dynamicSpawnSerial=Number.isInteger(options.dynamicSpawnSerial)?options.dynamicSpawnSerial:0;this.spawnSessionSeed=Number.isInteger(options.spawnSessionSeed)?options.spawnSessionSeed:0;this.recentSpawnLocations=[];
    ...
    {const ratio=this.player.hp/Math.max(1,this.player.maxHp);applyStats(this.player);this.player.hp=Math.min(this.player.maxHp,Math.max(1,Math.ceil(this.player.maxHp*ratio)));}
    this.enemies = [];
    ...
    this.refillWild(2);this.enemies.sort((a,b)=>Number(!!a.isBoss)-Number(!!b.isBoss)||Number(!!a.horde)-Number(!!b.horde));
  ...
  activeSnapshot(){const p=this.player;return Object.fromEntries(['captureId','id','name','element','level','xp','hp','maxHp','attack','defense','spAttack','spDefense','speed','movementSpeed','nature','ability','ivs','evs','attributePoints','attributes','attributeSystemVersion','moveLoadoutVersion','evolutionHistory','knownMoves','slots'].map(k=>[k,p[k]]));}
  ...
  captureDefeated(e){const ball=pokeballById(this.capturePlan.ballId);if(!this.capturePlan.enabled||this.capturePlan.speciesId!==e.id||!(this.inventory.items[ball.name]>0))return;this.inventory.items[ball.name]--;if(!this.inventory.items[ball.name])delete this.inventory.items[ball.name];const base=SPAWN_RARITIES[e.spawnRarity]?.captureChance||.35,chance=pokeballCaptureChance(base,e.level,ball.id,this.map.scene),roll=((Math.imul(e.uid+this.kills+1,2654435761)^this.map.seed)>>>0)/4294967296;this.emit('captureThrow',{name:e.name,chance,ball:ball.name});if(roll>chance){this.emit('captureFail',{name:e.name,chance,ball:ball.name});return;}const captureId=`capture-${++this.captureSerial}-${e.id}`,specimen={captureId,id:e.id,name:e.name,element:e.element,level:e.level,xp:0,attributePoints:0,evolutionHistory:[],knownMoves:[],slots:[null,null,null,null],hp:e.maxHp,rarity:e.spawnRarity,rarityName:e.spawnRarityName};applyStats(specimen);specimen.hp=specimen.maxHp;this.pc.push(specimen);this.emit('captured',{name:e.name,specimen,ball:ball.name});}
  selectCaptured(captureId){const specimen=this.pc.find(p=>p.captureId===captureId);if(!specimen||!CREATURES[specimen.id])return false;this.syncActivePokemon();const position={x:this.player.x,y:this.player.y},ratio=(specimen.hp||specimen.maxHp)/Math.max(1,specimen.maxHp);this.definition=CREATURES[specimen.id];this.player={...this.definition,...specimen,...position,hp:specimen.hp||specimen.maxHp,slots:specimen.slots||[null,null,null,null],knownMoves:specimen.knownMoves||[],cooldowns:{},buffs:[],path:[],facing:{x:0,y:1},dead:false,target:null,hurtbox:{radius:12,layer:32},collisionLayer:2};applyStats(this.player);this.player.hp=Math.min(this.player.maxHp,Math.max(1,Math.ceil(this.player.maxHp*ratio)));this.learn();this.emit('pokemonSelected',{name:specimen.name,level:specimen.level});return true;}
  learn() {
    const p=this.player,stages=[p.id];for(let i=0;i<Object.keys(CREATURES).length;i++){const previous=EVOLUTIONS.find(e=>e.targetCreatureId===stages[0]);if(!previous)break;stages.unshift(previous.creatureId);}
    const rows=[],available=new Set(),reservedSlots=new Set();for(const id of stages)for(const row of LEARNSETS[id]||[]){if(Number.isInteger(row.slot))reservedSlots.add(row.slot);if(row.level<=p.level&&ABILITIES[row.ability]&&!available.has(row.ability)){rows.push(row);available.add(row.ability);}}
    p.knownMoves=Array.isArray(p.knownMoves)?p.knownMoves:[];p.slots=Array.isArray(p.slots)?p.slots.slice(0,4):[];while(p.slots.length<4)p.slots.push(null);
    if(p.moveLoadoutVersion!==MOVE_LOADOUT_VERSION){p.knownMoves=p.knownMoves.filter(id=>available.has(id));p.slots=p.slots.map(id=>available.has(id)?id:null);p.moveLoadoutVersion=MOVE_LOADOUT_VERSION;}
    for(const row of rows)if(!p.knownMoves.includes(row.ability)){p.knownMoves.push(row.ability);if(Number.isInteger(row.slot)&&!p.slots[row.slot])p.slots[row.slot]=row.ability;this.emit('learn',{name:ABILITIES[row.ability].name,ability:row.ability});}
    const unlocked=p.level>=25?4:p.level>=10?3:2,used=new Set(p.slots.filter(Boolean)),remaining=p.knownMoves.filter(id=>available.has(id)&&!used.has(id));
    const power=id=>ABILITIES[id].power||ABILITIES[id].damage||0,damaging=id=>power(id)>0&&ABILITIES[id].behavior!=='buff'&&ABILITIES[id].behavior!=='debuff'&&ABILITIES[id].behavior!=='heal';
    for(let slot=0;slot<unlocked;slot++)if(!p.slots[slot]&&!reservedSlots.has(slot)){let index=slot===0?remaining.findIndex(damaging):slot===1?remaining.findIndex(id=>!damaging(id)):-1;if(index<0&&slot>=2){let best=-1;for(let i=0;i<remaining.length;i++)if(best<0||power(remaining[i])>=power(remaining[best]))best=i;index=best;}if(index<0)index=0;const [ability]=remaining.splice(index,1);if(ability){p.slots[slot]=ability;used.add(ability);}}
  }
  ...
  moveWithInput(dt){
    ...
    const speed=(p.movementSpeed||p.speed)*PLAYER_MOVEMENT_MULTIPLIER*(p.running?1.5:1)*(p.buffs||[]).reduce((value,b)=>value*(b.speedMultiplier||1),1);
  ...
  step(dt) {
    ...
    if((!this.map.scene||this.map.layer)&&this.time>=this.streamAt){this.streamCreatures();this.streamAt=this.time+this.spawnConfig.refillInterval;}
  ...
  applySharedWildState(state){
    ...
    if(!enemy&&state.dynamicSpawn&&state.id&&Math.hypot((state.x||0)-this.player.x,(state.y||0)-this.player.y)<=this.spawnConfig.radius){
      ...
      const occupied=this.enemies.some(e=>e.state!=='Dead'&&Math.hypot(e.x-state.x,e.y-state.y)<this.spawnConfig.entitySpacing);
  ...
  createWildEntity(spawn){
    ...
    const stats=encounterStats(species,this.map,spawn,spawn.uid),maxHp=Number(spawn.maxHp)||stats.maxHp;
    const entity=this.creaturePool.take({...species,...spawn,...stats,id:spawn.species||spawn.id,skills:enemySkillSet(species.element,spawn.level||stats.level),maxHp,hp:Number.isFinite(spawn.hp)?Math.max(0,Math.min(maxHp,spawn.hp)):maxHp,home:spawn.home||{x:spawn.x,y:spawn.y},state:spawn.state||'Idle',timer:.15+(spawn.uid%12)*.11,cooldown:Number(spawn.cooldown)||0,path:[],facing:spawn.facing||{x:0,y:1},moving:false,slowUntil:0,staggerUntil:0,dynamicSpawn:true});
    entity.hurtbox={radius:entity.radius,layer:32};entity.collisionLayer=4;entity.navigation={maxPathNodes:1500};
    this.enemies.push(entity);return entity;
  }
  refillWild(limit=this.spawnConfig.refillBatch){
    let created=0;
    for(let slot=0;slot<limit;slot++){
      const active=this.enemies.filter(enemy=>!enemy.isBoss&&!enemy.horde).length;if(active>=this.spawnConfig.targetPopulation)break;
      const serial=++this.dynamicSpawnSerial,spawn=findDynamicSpawn({map:this.map,player:this.player,enemies:this.enemies,recent:this.recentSpawnLocations,spawnEpoch:(this.spawnEpoch^this.spawnSessionSeed)>>>0,serial,now:this.worldNow(),config:this.spawnConfig});
      if(!spawn)continue;
      this.createWildEntity(spawn);created++;
    }
    return created;
  }
  streamCreatures() {
    ...
    this.enemies=near;
    const ids=new Set(near.map(e=>e.uid));

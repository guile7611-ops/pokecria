...
export class Renderer {
 ...
 draw(dt,active){const p=this.sim.player;if(this.worldView.map!==this.sim.map){this.worldView.dispose();this.worldView=new WorldView(this.sim.map);this.camera={x:p.x,y:p.y};}
  for(const e of this.remoteEntities.values()){if(!Number.isFinite(e.targetX)||!Number.isFinite(e.targetY))continue;const blend=1-Math.exp(-dt*12);e.x+=(e.targetX-e.x)*blend;e.y+=(e.targetY-e.y)*blend;if(Math.hypot(e.targetX-e.x,e.targetY-e.y)<.35){e.x=e.targetX;e.y=e.targetY;}}
  ...
  for(const [id,e] of this.remoteEntities)if(e.scene===scene&&e.rolling){actors.push(this.effect('moves/flameWheel',e.x,e.y,`flameWheel-${id}`,this.sim.time,72));actors.push(this.effect('pmd/0030',e.x,e.y,`flameWheel-pmd-${id}`,this.sim.time,94));}

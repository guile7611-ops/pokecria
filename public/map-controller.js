...
import { regionAt } from './map-generator.js';
export class MapController {
 ...
 draw(){if(performance.now()-this.lastDraw<(this.expanded?100:200))return;this.lastDraw=performance.now();this.measure();const sim=this.renderer.sim,p=sim.player;if(this.follow){this.x=p.x;this.y=p.y;}
  ...
  if(!this.atlas){const d=sim.discovery;for(let y=0;y<d.rows;y++)for(let x=0;x<d.cols;x++){const state=d.state(x*256+1,y*256+1);if(state!=='CURRENT')box('map-fog '+(state==='DISCOVERED'?'discovered':''),x*256,y*256,256,256);}}

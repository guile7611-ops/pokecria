...
export class WorldView {
 ...
 render(host,view,{objects=true,actors=[]}={}){const state=this.state(host),{x,y,zoom,width,height}=view,left=x-width/zoom/2,top=y-height/zoom/2,right=x+width/zoom/2,bottom=y+height/zoom/2,lod=zoom>.3?0:zoom>.09?1:2;
  state.frame++;
  state.plane.style.transform=`translate3d(${width/2-x*zoom}px,${height/2-y*zoom}px,0) scale(${zoom})`;
  for(let cy=Math.max(0,Math.floor(top/768));cy<=Math.min(Math.ceil(this.map.rows/24)-1,Math.floor(bottom/768));cy++)for(let cx=Math.max(0,Math.floor(left/768));cx<=Math.min(Math.ceil(this.map.cols/24)-1,Math.floor(right/768));cx++){
   ...
   if(!node.firstChild){const img=document.createElement('img');img.alt='';img.decoding='async';img.draggable=false;img.src=`/api/tile-chunk?rev=encounters-4&seed=${this.map.seed}&cx=${cx}&cy=${cy}&lod=${lod}&layer=${layer}&scene=${encodeURIComponent(this.map.scene||'')}`;node.append(img);}
  }
  ...
  for(const [key,node]of state.nodes)if(node._seenFrame!==state.frame){node.remove();state.nodes.delete(key);}

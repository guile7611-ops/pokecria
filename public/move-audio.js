...
import {PXG_MOVE_AUDIO} from './pxg-attack-vfx.js';
...
export async function playMoveAudio(context,ability,volume=.12){
 ...
 const pxg=PXG_MOVE_AUDIO[ability.id],exact=EXTERNAL_MOVE_VFX[ability.id]?.audio,path=pxg?`/assets/audio/moves/pxg/${pxg}`:exact?`/assets/audio/moves/external/${exact}`:null;

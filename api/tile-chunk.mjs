...
export default async function handler(req,res){
  try{
    ...
    if(!Number.isInteger(seed)||seed<0||seed>4294967295||!Number.isInteger(cx)||cx<0||cx>20||!Number.isInteger(cy)||cy<0||cy>15||![0,1,2].includes(lod)||!['ground','overview'].includes(layer)||String(scene).length>100){res.status(400).end('Invalid tile chunk');return;}

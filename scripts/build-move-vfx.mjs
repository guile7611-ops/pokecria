...
function draw(motif,t,p,s){
 ...
 case 'growl':return [0,1,2].map(i=>Q(`M ${15+i*10} ${21-i*6} Q ${31+i*7+t*2} 32 ${15+i*10} ${43+i*6}`,i%2?a:b,3)).join('')+C(10,32,5,d);
 case 'leer':return `<ellipse cx="32" cy="32" rx="25" ry="${12+t}" fill="${d}"/>`+C(32,32,8,b)+C(32,32,4,a)+L(8,14,22,20,a,3)+L(56,14,42,20,a,3);
 case 'screech':return Q(`M 2 32 L 12 ${25-t} L 20 42 L 27 12 L 35 51 L 44 23 L 52 39 L 62 31`,b,5)+Q(`M 2 32 L 12 ${25-t} L 20 42 L 27 12 L 35 51 L 44 23 L 52 39 L 62 31`,a,2);
 case 'scaryFace':return C(32,32,24,d)+P('10,20 27,26 23,32',a)+P('54,20 37,26 41,32',a)+Q('M 19 44 Q 32 32 45 44',b,4);
 case 'hypnosis':return Array.from({length:4},(_,i)=>`<ellipse cx="32" cy="32" rx="${7+i*6+t}" ry="${6+i*4}" fill="none" stroke="${i%2?a:b}" stroke-width="2" transform="rotate(${t*25+i*35} 32 32)"/>`).join('');
 case 'toxic':return P(`32,5 48,31 45,48 32,57 19,48 16,31`,d)+P('32,17 41,34 32,47 23,34',b)+C(32,33,5,a)+dots(6,23,b);
 case 'yawn':return Q(`M 16 17 Q 31 8 48 17 Q 54 42 32 52 Q 10 42 16 17`,b,3)+C(32,33,7,d)+[0,1,2].map(i=>C(44+i*5,19-i*5,2+i%2,a)).join('');
 case 'leechSeed':return C(32,33,11,d)+C(29,30,5,b)+Q('M 32 34 Q 5 24 14 9 M 32 34 Q 51 12 54 4 M 32 34 Q 62 53 53 61',b,4)+P('14,8 5,7 12,18',a)+P('53,6 62,8 55,17',a);
 case 'amnesia':return C(32,32,22,d)+Q(`M 21 37 Q 12 21 29 18 Q 43 ${10+t} 45 27 Q 53 43 34 48 Q 19 48 21 37`,a,4)+Q('M 27 21 Q 41 30 27 43',b,3);
 case 'defenseCurl':return C(32,32,21,d)+C(32,32,16,b)+Q('M 18 33 Q 30 14 45 30 Q 42 48 25 43',a,5)+C(40,27,3,d);
 case 'lightScreen':return R(12,8,40,48,d)+R(16,12,32,40,b)+[0,1,2].map(i=>L(17,18+i*12,47,18+i*12,a,2)).join('')+L(32,12,32,52,a,2);
 case 'detect':return `<ellipse cx="32" cy="32" rx="26" ry="14" fill="none" stroke="${b}" stroke-width="5"/>`+C(32,32,10,d)+C(32,32,5,a)+spokes(4,24,30,a,2,t*.05);
 case 'focusEnergy':return C(32,32,9,d)+C(32,32,4,a)+[0,1,2].map(i=>`<circle cx="32" cy="32" r="${13+i*7+t}" fill="none" stroke="${i%2?a:b}" stroke-width="2"/>`).join('');
 case 'rainDance':return Array.from({length:9},(_,i)=>{const x=8+i*6,y=(i*9+t*7)%58;return L(x,y,x-4,y+10,i%2?a:b,3)}).join('')+Q('M 8 20 Q 20 3 35 12 Q 51 6 58 20',d,5);
 case 'doubleEdge':return P(`3,31 28,12 26,25 57,25 57,38 26,38 28,51`,d)+P(`10,31 25,21 28,29 53,29 53,34 28,34 25,42`,a)+spokes(7,15,r+12,b,3);
 case 'suckerPunch':return P(`8,24 30,22 32,12 50,16 56,33 43,48 17,46`,d)+P('10,28 32,27 41,21 50,27 47,38 25,41',b)+L(5,45,22,38,a,3);
 case 'aquaTail':return Q(`M 6 44 Q 8 8 34 20 Q 60 29 49 53`,b,10)+Q(`M 7 44 Q 8 8 34 20 Q 60 29 49 53`,a,4)+dots(6,27,b);
 case 'bodySlam':return C(32,30,15+t,d)+C(28,24,7,b)+P('13,43 24,53 44,53 54,43',a)+L(10,54,54,54,b,3);
 case 'pound':return P(`22,5 42,5 45,35 38,47 22,44 17,35`,d)+P('25,9 37,9 40,33 33,42 24,39',b)+L(9,51,55,51,a,4);
 case 'fireFang':return P(`10,8 24,15 26,36 18,39`,b)+P(`54,8 40,15 38,36 46,39`,b)+P(`11,56 25,49 27,30 19,26`,a)+P(`53,56 39,49 37,30 45,26`,a)+C(32,32,8,d);
 case 'furySwipes':return [0,1,2,3].map(i=>Q(`M ${8+i*12} 51 Q ${12+i*12+t} 32 ${19+i*11} 11`,i%2?a:b,4)).join('');
 case 'bubbleBeam':return Array.from({length:8},(_,i)=>{const x=8+i*7,y=32+Math.sin(i+t*.4)*11;return `<circle cx="${x}" cy="${y}" r="${3+i%3}" fill="none" stroke="${i%2?a:b}" stroke-width="2"/>`}).join('');
 case 'confusion':return Q(`M 6 32 Q 18 ${10+t} 31 32 Q 43 ${50-t} 58 32`,b,7)+Q(`M 6 32 Q 18 ${10+t} 31 32 Q 43 ${50-t} 58 32`,a,2)+C(32,32,6,d);
 case 'thunderShock':return P(`27,5 14,35 29,32 22,58 50,23 34,28 42,5`,b)+P('29,10 21,31 34,27 28,46 43,26 31,31 37,10',a);
 case 'discharge':return C(32,32,8,d)+spokes(11,9,r+13,b,5,t*.13)+spokes(11,10,r+10,a,2,t*.13);
 case 'electroBall':return C(32,32,r+2,d)+C(29,28,r*.7,b)+C(26,25,r*.25,a)+spokes(7,r+2,r+14,a,3,t*.2);
 case 'harden':return P('32,5 53,17 57,39 32,58 7,39 11,17',d)+P('32,11 48,21 51,37 32,51 13,37 16,21',b)+L(16,21,48,21,a,3);
 case 'ironDefense':return P('32,3 57,15 60,42 32,61 4,42 7,15',d)+P('32,10 51,20 53,38 32,53 11,38 13,20',b)+L(11,38,53,38,a,4)+L(32,10,32,53,a,2);
 case 'bulkUp':return P('8,52 14,30 25,28 27,15 37,15 39,28 50,30 56,52',d)+P('14,49 19,33 28,32 32,27 36,32 45,33 50,49',b)+spokes(5,14,r+13,a,3,t*.13);
 case 'teleport':return C(32,32,8,a)+[0,1,2].map(i=>`<circle cx="32" cy="32" r="${10+i*7+t*2}" fill="none" stroke="${i%2?a:b}" stroke-width="3" stroke-dasharray="8 4"/>`).join('');
 case 'poisonSting':return P('4,30 47,23 60,32 47,41',d)+P('9,30 50,28 58,32 50,36',a)+C(18+t*3,32,3,b);
 case 'stringShot':return [0,1,2,3].map(i=>Q(`M 3 ${10+i*14} Q ${25+t*2} ${28+i*3} 60 ${10+i*14}`,i%2?a:b,3)).join('')+C(31,32,5,d);
 case 'bugBite':return P(`12,${8+t} 25,18 31,34 23,47`,a)+P(`52,${8+t} 39,18 33,34 41,47`,b)+C(32,33,8,d);
 case 'stunSpore':return dots(22,9+r,b)+[0,1,2].map(i=>C(14+i*17,25+(i%2)*13,5,i%2?a:d)).join('');
 case 'sleepPowder':return dots(18,13+r,a)+Q('M 16 21 Q 32 46 51 19',b,3)+C(48,15,4,d);
 case 'supersonic':return [0,1,2,3].map(i=>Q(`M ${9+i*9} 9 Q ${1+i*9+t} 32 ${9+i*9} 55`,i%2?a:b,3)).join('')+L(5,32,59,32,d,2);
 case 'aerialAce':return P(`3,34 35,8 25,28 57,19 35,55 41,37`,b)+P('10,34 31,15 29,32 48,25 34,47 36,34',a)+L(5,55,23,43,d,3);
 case 'gust':return Q(`M 6 38 Q 20 11 39 28 Q 55 44 39 53 Q 22 58 18 42`,b,6)+Q(`M 9 38 Q 22 17 40 29 Q 50 40 38 48`,a,2)+dots(4,21,d);
